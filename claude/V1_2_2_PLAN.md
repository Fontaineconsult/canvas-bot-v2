...
1. **Content Viewer** -- after scanning a course, browse all discovered content (documents, videos, audio, images, unsorted) in organized tables with clickable URLs
...
### Step 0.2: Refactor `gui/app.py` -- CTkTabview with "Run" tab
...
**Verify:** Launch GUI. One "Run" tab visible. All existing functionality works identically: course ID entry, browse, checkboxes, run, log output, keyboard shortcuts (Alt+R/V/C/A), settings persistence.
...
## Phase 1: Content Discovery Viewer

### Step 1.1: Wire content data in controller

In `gui/controller.py`:

- Add `self._last_content_data = None` in `__init__`
- In `_run_worker()`, after `bot.start()`, capture: `self._last_content_data = bot.content_extractor.get_all_content()`
- After processing loop, schedule: `self.view.root.after(0, self._on_content_ready)`
- Stub `_on_content_ready()` method

For batch mode (course list), capture content from each course, so `_last_content_data` holds the final one.

**Verify:** Add temp print in `_on_content_ready`. Run scan. Confirm print appears in log.

---

### Step 1.2: Create `gui/content_viewer.py` -- skeleton

`ContentViewer` class taking `parent_frame`:

- **Summary banner** at top: course name label + stats label
...
- `load_data(content_dict)` method -- clears all, populates summary + tables
- `clear()` method -- resets to placeholder state
...
- Replace Content tab placeholder with `self.content_viewer = ContentViewer(self.tabview.tab("Content"))`

In `gui/controller.py`:

- `_on_content_ready()` calls `self.view.content_viewer.load_data(self._last_content_data)` and auto-switches to Content tab via `self.view.tabview.set("Content")`

**Verify:** Run a course scan. GUI auto-switches to Content tab. Sub-tabs appear. Tables populated with data. Row counts match JSON file. Column sorting works. Scrolling works.
...
- For `url`, `source_page_url`, and `save_path` fields: make them clickable
- URL click opens in default browser via `webbrowser.open(url)`

**Verify:** Click a row in any table. Detail panel shows all fields. Click a URL in the detail panel -- browser opens that URL.

---

### Step 1.5: Summary statistics banner

In `load_data()`, compute counts per category and display:
...
### Step 1.6: Clear content on new scan start

In `gui/controller.py` `on_run()`:

- Set `self._last_content_data = None`
- Call `self.view.content_viewer.clear()` if it exists

**Verify:** Run scan, see content. Start new scan. Content tab shows "No content loaded" during scan. After completion, fresh data appears.

---

## Phase 2: Pattern Manager
...
2. **Inline validation** on submit: `re.compile(pattern)` -- show error in red if invalid
...
- On test: `importlib.reload(sorters.sorters)` to pick up any edits saved to `%APPDATA%\canvas bot\re.yaml`, then test input against all compiled matchers
- Show matches in green or "No matches (Unsorted)" in orange

Reload flow (patterns live in AppData, sorters reads them at import time):

```python
import importlib
import sorters.sorters
importlib.reload(sorters.sorters)
# Now import fresh matchers from the reloaded module
...
Step 0.3 (placeholder tabs)
    ↓
Step 1.1 (wire content data)
    ↓
Step 1.2 (ContentViewer skeleton)
    ↓
Step 1.3 (integrate viewer)
    ↓
Step 1.4 (detail panel + clickable URLs)
    ↓
Step 1.5 (summary stats)
    ↓
Step 1.6 (clear on re-scan)
    ↓
...
- **Phase 1:** Scan course 64634, Content tab shows all data matching `test_data/64634.json`
